...
fun MuseTheme(darkTheme: Boolean = isSystemInDarkTheme(), content: @Composable () -> Unit) {
    ...
    val fontScale = configuration.fontScale.coerceIn(0.8f, 1.3f)

    val restrictedConfiguration = Configuration(configuration).apply {
        this.fontScale = fontScale
    }

    val colorScheme = if (darkTheme) DarkColorScheme else LightColorScheme

    CompositionLocalProvider(LocalConfiguration provides restrictedConfiguration) {
        MaterialTheme(
