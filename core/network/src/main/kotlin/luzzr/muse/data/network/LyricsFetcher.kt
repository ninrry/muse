 ...
 */
class LyricsFetcher : LyricsSearchClient {
    ...
    private fun searchQQSong(title: String, artist: String?): QQSongMatch? {
        ...
        val connection = openQQConnection(url)
        return try {
            if (connection.responseCode != HttpURLConnection.HTTP_OK) return null
            val response = connection.inputStream.bufferedReader().use { it.readText() }
            val songs = JSONObject(cleanJsonp(response))
                .takeIf { it.optInt("code", -1) == 0 }
                ?.optJSONObject("data")
                ?.optJSONObject("song")
                ?.optJSONArray("list")
                ?: return null
            pickBestQQSong(songs, title, artist)
        } finally {
    ...
    private fun pickBestQQSong(songs: org.json.JSONArray, title: String, artist: String?): QQSongMatch? {
        ...
        return match.first.takeIf {
            match.second >= SearchMatch.minimumAcceptableScore(artist) ||
                SearchMatch.titleScore(title, it.title) >= 34
        }
    }
    ...
    private fun fetchQQLyrics(songMid: String): String? {
        val url = URL(
        ...
        return try {
            if (connection.responseCode != HttpURLConnection.HTTP_OK) return null
            val response = connection.inputStream.bufferedReader().use { it.readText() }
            val base64Lyrics = JSONObject(cleanJsonp(response))
                .takeIf { it.optInt("code", -1) == 0 }
                ?.optString("lyric", "")
    ...
    private fun openQQConnection(url: URL): HttpURLConnection {
