...
class ReadAlongRepositoryImpl @Inject constructor(
    ...
    override fun observeBooks(): Flow<List<ReadAlongBook>> =
        dao.observeBooks().map { rows -> rows.map(::toDomain) }
    ...
    private fun importArchive(source: ReadAlongImportSource): ReadAlongImportResult {
        ...
        try {
            ...
            require(manifestFile.isFile) { "同步书包缺少 manifest.json" }
            ...
            return commitImportedBook(staging, parsed, metadata, manifest)
        } finally {
            temporary.delete()
    ...
    private fun parseEpub(
        ...
        ZipFile(epub).use { zip ->
            ...
            val coverPath = cover?.path?.let { path ->
                val extension = path.substringAfterLast('.', "jpg").take(8)
    ...
    private fun bindLooseAudio(parsed: ParsedEpub, root: File, newlyCopied: List<File>): ParsedEpub {
        val allAudio = File(root, AUDIO_DIR).listFiles().orEmpty().filter { it.isFile && it.extension.lowercase(Locale.ROOT) in AUDIO_EXTENSIONS }
            ...
            if (chapter.audioPath != null) {
                ...
                if (match != null) {
                    ...
                    chapter.copy(
                        ...
                        audioDurationMs = mediaDuration(match),
    ...
    private fun parseSentence(json: JSONObject, chapterId: String, chapterHref: String): ReadAlongSentence? {
        ...
        val unitsJson = json.optJSONArray("unit_timings") ?: return null
        val units = buildList {
    ...
    private fun resolveHref(base: String, href: String): Pair<String, String?> {
        ...
        val decoded = runCatching { URI(rawPath).path }.getOrNull().orEmpty().ifBlank { rawPath }
