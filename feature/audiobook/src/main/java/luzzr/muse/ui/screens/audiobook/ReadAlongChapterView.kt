...
fun ReadAlongChapterView(
    ...
    LaunchedEffect(pageReady, readerSettings, annotations, jumpMode, chromeVisible) {
        if (pageReady) {
            webView.evaluateJavascript(buildSetupScript(readerSettings, annotations, jumpMode), null)
        }
    }
    ...
    LaunchedEffect(pageReady, targetTextRange, chapterData.chapter.href) {
        ...
        if (pageReady) {
            webView.evaluateJavascript(buildScrollToTextRangeScript(target.first, target.last + 1), null)
            onTextRangeConsumed()
        }
    }
...
internal fun buildScrollToTextRangeScript(start: Int, end: Int): String {
          ...
          if (!window.__museReadAlongClickInstalled) {
            ...
            document.addEventListener('click', (event) => {
              ...
              if (window.__musePaged && event.clientX < width * 0.28) {
                ...
                if (scroller) scroller.scrollBy({ left: -width, behavior: 'smooth' });
                return;
              }
            ...
            document.addEventListener('touchend', (event) => {
              ...
              scroller.scrollTo({ left: targetPage * width, behavior: 'smooth' });
              ...
              scrollTimer = setTimeout(() => {
                ...
                if (paged && max > 0 && viewport > 0) {
                  ...
                  if (Math.abs(target - current) > 2) {
                    snapTimer = setTimeout(() => scroller.scrollTo({ left: target, behavior: 'smooth' }), 80);
                  }
                }
              }, 120);
            };
